...
$descriptions = explode('<br />'.PHP_EOL.'<br />', $descriptions);
...
for ($i = 0; $i < 50; $i++) {
	...
	$rand_month = sprintf('%1$02d', rand(1, 12));
	...
	$first_point = strpos($description, '.');

	$name = substr($description, 0, $first_point);
	$description = substr($description, $first_point + 2);
	$category = rand(1 , 5);
	$price = rand(10 , 100);
...
foreach($products as $key => $product) {
	...
	$query = $db->prepare('INSERT INTO products (name, description, date) VALUES (:name, :description, :date)');
	$query->bindValue('date', $product['date']);
...
$query = $db->prepare('INSERT INTO products (name, description, date, category, price, picture, rating) VALUES (:name, :description, :date, :category, :price, :picture, :rating)');
...
foreach($products as $key => $product) {
	...
	$result = !empty($last_insert_id) ? 'OK' : 'NOK';

	echo $result.'|'.$date.'|'.$description.'|'.$name.'<br>';
}